...
public class GM_IT_DAO {
	
	public static int insertIntoProduct(Articolo a){
		Log.debug("Inserimento articolo su gloriamoraldi.it ...");
		Connection con = null;
		...
		try {			
			...
			if (a.getIdArticolo()!=0 /* && !checkIfProductExist(a.getIdArticolo(),con,ps) */){
				...
				int id_iva = 16; /* 22% */ 
				if (a.getAliquotaIva()==10) id_iva=17; /* 10% */ 
				else if (a.getAliquotaIva()==4) id_iva=18; /* 4% */ 
...
//	}
	...
	private static List<Long> relatedProductsGenerator(Articolo a, PreparedStatement ps){
		...
		try{
			...
			query = "select distinct a.id_articolo, a.nome, " +
