...
public class GloriaMoraldi_DAO {

	public static int insertIntoProducts(Product p){
		Log.debug("Inserimento articolo su gloriamoraldi.it locale...");
		Connection con = null;
			...
			con = DataSource.getLocalConnection();
				...
				if (p.getProduct_id()!=0 && !checkIfProductExist(p.getProduct_id(),con)){
					...
					String query = "INSERT INTO `pmqbpiom_jshopping_products` (`product_id`, `parent_id`, `product_ean`, `product_quantity`, `unlimited`," +
						...
						" `meta_title_it-IT`, `meta_description_it-IT`, `meta_keyword_it-IT`, `extra_field_1`, `extra_field_2`) VALUES " +
						"(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
...
//				Methods.ScaricaImmagineECreaThumbnails(nome, cartella);
	...
	public static int deleteProduct(long idArticolo){	
		Log.debug("Eliminazione articolo con ID "+idArticolo+" da GloriaMoraldi.it ...");
		PreparedStatement ps = null;
		Connection con = null;
		int res = 0;
		...
		Log.debug("Fine eliminazione articolo da GloriaMoraldi.it");
		return res;
	}
